...
univerAPI.onBeforeCommandExecute(command => {
 if (command.id === 'sheet.operation.set-cell-edit-visible') {
  if ((command.params as { visible: boolean }).visible) {
    univerAPI.executeCommand('sheet.operation.set-cell-edit-visible', { eventType: DeviceInputEventType.PointerUp, visible: false })
